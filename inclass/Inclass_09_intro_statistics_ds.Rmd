...
ozone.wide <- ozone.wide %>%
              mutate(House.Number = factor(House.Number, levels = ave.io.house$House.Number, ordered = TRUE))

ozone.wide <- ozone.wide %>%
              mutate(House.Number = fct_reorder(House.Number, In_Out_ratio, .fun=mean))

ggplot(data = ozone.wide,aes(x = House.Number, y = In_Out_ratio,fill=House.Number)) + 
  geom_jitter(size=2,alpha=0.9,width=0.2,pch=21,color='black')+
  theme_bw()+
   labs(x='House', y= 'I/O')+
  facet_grid(.~`Type of Air Conditioner`, scales='free') +
...
```{r}
ozone.wide <- ozone.wide %>%
              mutate(ac.type = `Type of Air Conditioner`)



ave.io.type <- ozone.wide %>%
               group_by(ac.type) %>%
               summarize(ave.io = mean(In_Out_ratio, na.rm=TRUE),
                         sd = sd(In_Out_ratio),
                         n = sum(!is.na(In_Out_ratio))) %>%
                mutate(tcrit = qt(.975,df=(n-1))) %>%
                mutate(bound = tcrit * sd/sqrt(n)) %>%
                mutate(lower.bound = ave.io - bound) %>%
                mutate(upper.bound = ave.io + bound) 
...
ggplot(data=ave.io.type, aes(x=ac.type, y=ave.io, fill=ac.type)) +
  geom_col()+
  geom_errorbar(aes(ymin = lower.bound, ymax = upper.bound), width=0.3)
...
- We assume the our population is approximately normal (although the t-distribution is fairly robust to departures of normality if our n is large)
  - What is our N?
  - we have 20 and 25 observations. We have a small sample
...
```{r}
hg <- ggplot(data=ozone.wide, aes(x=In_Out_ratio, fill=ac.type))+
...
Add an order sequence
Calculate the percentile of the order
...
qq <- ggplot(data=ozone.wide, aes(sample=In_Out_ratio, color=ac.type))+
  geom_qq(aes(sample=In_Out_ratio))+
  geom_qq_line()+
  facet_wrap(~ac.type,scales='free')
...
- Let's say  we kept kept re-sampling I/O ratios from the same homes (perhaps each home 100 times). So our N became very large. What would happen to our confidence intervals? 
- Decrease by a factor of 1/sqrt(n). 
- However, we still would be limited to the same number of homes.... 
...
- No-- because we are still sampling the same 30 homes, we haven't expanded our sample of different types of homes
...
ozone.mean.home <- ozone.wide %>%
                   group_by(House.Number, ac.type) %>%
                   summarize(mean_I_O = mean(In_Out_ratio),
                             visits = n())
...
```{r}
ave.io.type.2 <- ozone.mean.home %>%
               group_by(ac.type) %>%
               summarize(ave.io = mean(mean_I_O, na.rm=TRUE),
                         sd = sd(mean_I_O),
                         n = sum(!is.na(mean_I_O))) %>%
                mutate(tcrit = qt(.975,df=(n-1))) %>%
                mutate(bound = tcrit * sd/sqrt(n)) %>%
                mutate(lower.bound = ave.io - bound) %>%
                mutate(upper.bound = ave.io + bound) 

kable(ave.io.type.2,digits=2)
...
ggplot(data=ave.io.type.2, aes(x=ac.type, y=ave.io, fill=ac.type)) +
  geom_col()+
  geom_errorbar(aes(ymin = lower.bound, ymax = upper.bound), width=0.3)
...
names(ozone.mean.home)

```{r}

t.test.ozone <- t.test(mean_I_O ~ ac.type, data=ozone.mean.home)
