...
Look at coefficients when we fit the models separately
...
##intercept term for EC model
summer.coef[1,2]+summer.coef[3,2]

## the slope for the EC model
summer.coef[2,2] + summer.coef[4,2]


Sidepak.summer.EC <- SidePak.visit.sum %>%
                  filter(season == 'Summer') %>%
                  filter(ac.type == 'EC')

## fit a lm model to the summer data, predicting the In concentration from the Outdoor concentration

summer.ec.lm <- lm(formula = In_mean ~ Out_mean, data=Sidepak.summer.EC)

summary(summer.ec.lm)


```


Show class how coefficients work
Show them that we get the same answer whether we combine the data by ac.type, or fit separate linear models for each one...
...
```{r}
SidePak.summer.no.fire <- SidePak.visit.sum %>%
...
summer.no.fire.lm <- lm(formula = In_mean ~ Out_mean + ac.type + Out_mean:ac.type, data=SidePak.summer.no.fire)

## use get_regression_coeff
  
ozone.coeff.summer.no.fire <- summer.no.fire.lm %>%
                              get_regression_table() %>%
                              mutate(id = 'summer no fire')
...
predict.summer.no.fire <- summer.no.fire.lm %>%
                          augment(data = SidePak.summer.no.fire, interval = "confidence")
...
```{r}
ggplot(data=predict.summer.no.fire, aes(y=.resid, x=.fitted)) +
  geom_point(alpha=0.5)+
  geom_hline(yintercept=0)+
  theme_bw()
```
