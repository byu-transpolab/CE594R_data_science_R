...
4.  Working directory. By default the working directory within a code chunk is where the .RMD file is located. Whereas .R files are by default to the .Rproj file. You can change the default working directory to be the the location of the .Rproj, go to tools -> Global Options -> R Markdown. 
<https://bookdown.org/yihui/rmarkdown-cookbook/working-directory.html>
5.  (Bonus) Scripts do not have the header at the top (called a YAML), but Markdown files do.
